    ...
    <>
      <header
        className={clsx(
          ...
          isScrolled ? 'bg-primary-dark/90 backdrop-blur-md py-4 border-b border-white/5' : 'bg-transparent py-6'
        )}
      >
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
          {/* Logo */}
          <Link to="/" className="text-2xl tracking-widest font-serif text-text-primary uppercase group">
            Solvara <span className="text-gold group-hover:text-gold-rich transition-colors">Safaris</span>
          </Link>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={clsx(
                  ...
                  location.pathname === link.path ? 'text-gold' : 'text-text-primary'
                )}
              >
                {link.name}
              </Link>
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <Link
              to="/expeditions"
              className="px-6 py-2 border border-gold text-gold hover:bg-gold hover:text-primary-dark transition-all duration-300 tracking-widest text-sm uppercase"
            >
              Plan Safari
            </Link>
          </div>
        ...
        {mobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col items-center space-y-6 text-lg tracking-widest uppercase">
              ...
              <Link
                to="/expeditions"
                className="mt-6 px-8 py-3 border border-gold text-gold hover:bg-gold hover:text-primary-dark transition-all duration-300"
              >
                Plan Safari
              </Link>
            </nav>
